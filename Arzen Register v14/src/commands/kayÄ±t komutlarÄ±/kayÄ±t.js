...
run: async (client, message, args, embed, prefix) => { 
    ...
    {
    message.react(red)
    message.lineReply(`\`${prefix}kayıt <@Üye/ID> <Isim> <Yaş>\``).then(x=>x.delete({timeout:5000})) 
    return }
   ...
   const tagModedata = await regstats.findOne({ guildID: message.guild.id })
    ...
    if(!uye.roles.cache.has(ayar.ekipRolu) && !uye.roles.cache.has(ayar.vipRole) && !uye.roles.cache.has(ayar.boosterRolu)) return message.lineReply(embed.setDescription(`${uye.toString()} isimli üye Taglı, Booster ve Vip olmadığı için isim değiştirmekden başka kayıt işlemi yapamazsınız.`));
    }

    if(!yaş) 
    { setName =`${Ayarlar.ikinciag} ${isim}`;
    } else { setName = `${Ayarlar.tag} ${isim} ' ${yaş}`;
  }
...
Üyesinin önceki isimlerine \`.isimler <@Üye/ID>\` komutuyla bakarak kayıt işlemini gerçekleştirmeniz önerilir.
    `)
...
.setFooter(`Lütfen 30 saniye alttaki butonlara basarak kullanıcının cinsiyetini belirleyin.`)
 ...
 let msg = await message.channel.send({ buttons : [ button_1, button_2, button_3 ], embed: artdx})
...
msg.edit({components: null, embed: artik}); 
